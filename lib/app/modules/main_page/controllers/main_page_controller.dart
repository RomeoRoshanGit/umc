...
class MainPageController extends GetxController {
  ...
  void setRuns(int run){
    if(run==7){
      balls[overChange]['Over ${overChange+1}'][ballChange]['runs'].value="W";
    }
    else if(run==8){
      balls[overChange]['Over ${overChange+1}'].add({
        "runs":"".obs
      });
      balls[overChange]['Over ${overChange+1}'][ballChange]['runs'].value="E";
      balls[overChange]['total']+=1;
      totalRuns.value+=1;
    }
    else{
      balls[overChange]['Over ${overChange+1}'][ballChange]['runs'].value=run.toString();
    ...
    if(overChange+1==balls.length && ballChange==balls[overChange]["Over ${overChange + 1}"].length){
      ...
      Get.defaultDialog(
        ...
        confirm: Padding(
          ...
          child: GestureDetector(
            onTap: (){
              Get.back();
              Get.toNamed(Routes.NEXT_INNINGS,arguments: [totalRuns,over]);
            },
            child: Expanded(
              child: Container(
                width: 110,
                ...
                child: Center(child: Text("2nd Innings",style: textMedium(color: ColorRes.bgColor),)),
              ),
            ),
          ),
        ),
          ...
          child: Expanded(
            child: GestureDetector(
              ...
              child: Container(
                ...
                child: Center(child: Text("Cancel",style: textMedium(color: ColorRes.ballColor),),),
              ),
            ),
          ),
        ),
